function a = cellaverages(x)
% CELLAVERAGES cell-medians
%   a = CELLAVERAGES(x) where x is an m x n matrix (possibly m==1)
%   returns an m x (n-1) matrix of average values between each two columns
%   a(i,j) = (x(i,j) + x(i,j+1))/2;
%
%   See also: CELLSIZES
...
leftpoints = [1:n-1];
rightpoints = [2:n];
a = (x(:,leftpoints) + x(:,rightpoints))/2;
