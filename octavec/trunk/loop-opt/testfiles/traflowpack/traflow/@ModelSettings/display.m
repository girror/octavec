function display(m)
%DISPLAY    Display a ModelSettings object
%   DISPLAY(m) displays the ModelSettings m
...
if length(m.info)==0
  stg = sindent(sprintf('%s based on ''%s.m'':\n', m.descriptor, m.f_msettings), 1);
  stg = [stg, sindent(sprintf('§ %s\n', strrep(m.desc, sprintf('\n'), [spad(sprintf('\n'), 2), '§ '])), 2),sprintf('\n')];
  stg = [stg, settingsInfo(m)];
