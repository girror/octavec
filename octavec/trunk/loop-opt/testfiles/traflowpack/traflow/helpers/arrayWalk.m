function result = arrayWalk(f, var, varargin)
%ARRAYWALK Apply function to all elements of an array
%   result = ARRAYWALK(f, arr, varargin) applies f to each element of f,
%   varargin can contain any further arguments that f requires.
%   (the first argument of f is always an element of arr).
...
result = [];
for i = 1:n
    result = [result, feval(f,var(i), varargin{:})];
end
